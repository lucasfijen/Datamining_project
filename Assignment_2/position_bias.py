...
def get_corrected_gain(df, correction_df=None):
    if correction_df != None:
        ...
        correction_df = pd.concat([random_clicks.rename('random clicks'), nonrandom_clicks.rename('non_random_clicks'), random_bookings.rename('random bookings'), nonrandom_bookings.rename('nonrandom bookings'), clicks_correction, bookings_correction], axis=1)
    ...
    corrected_gain = pd.DataFrame(df[['srch_id', 'position']].copy())
    ...
    for p in range(max(correction_df.position.values)):
        click_correction = correction_df[correction_df['position'] == p]['random/nonrandom clicks'].values.squeeze()
        book_correction = correction_df[correction_df['position'] == p]['random/nonrandom bookings'].values.squeeze()
